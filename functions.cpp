...
std::vector<unsigned char> ray_trace(std::vector<double> cam, std::vector<double> dir,std::vector<Figure*> Figures,std::vector<double> light){
            ...
            if (Figures[i]->ray_intersection(cam,dir,&result)==1){
                if (std::fabs(result-nearest)<std::numeric_limits<double>::epsilon()){
                    ...
                    for (size_t j = 0; j < col.size(); j++) {     
                        col[j] = (Figures[i]->color()[j]) * model_fong;
                       /* if(model_fong < 1.28165){ 
                            col[j] = (Figures[i]->color()[j]) * model_fong; 
                        }
                        else
                        col[j] = (Figures[i]->color()[j])* (model_fong);*/

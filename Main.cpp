...
int main() {
    ...
    Our_map["tetraedr"] = new Factory_tetra;
    ...
    while (!inputFile_2.eof()) {
        ...
        if (line == "sphere") {
            ...
            std::vector<unsigned char > color = { 255,192,203 };
            Figures.push_back(curFactory->second->Create(R, centre, color));
        }
        if (line == "box") {
            ...
            std::vector<unsigned char > color = { 148,0,211 };
            Figures.push_back(curFactory->second->Create(min, max, color));
        }
        if (line == "tetraedr") {
            double x, y, z, x1, y1, z1, x2, y2, z2, x3, y3, z3;
            inputFile_2 >> x >> y >> z >> x1 >> y1 >> z1 >> x2 >> y2 >> z2 >> x3 >> y3 >> z3;
            if (abs(x) > limit || abs(y) > limit || abs(z) > limit || abs(x1) > limit || abs(y1) > limit || abs(z1) > limit || abs(x2) > limit || abs(y2) > limit || abs(z2) > limit || abs(x3) > limit || abs(y3) > limit || abs(z3) > limit) {
                std::cout << "wrong data in tetraedr!";
                exit(0);
            }
            std::vector<double> p1 = { x,y,z };
            std::vector<double> p2 = { x1,y1,z1 };
            std::vector<double> p3 = { x2,y2,z2 };
            std::vector<double> p4 = { x3,y3,z3 };
            std::vector<unsigned char > color = { 8,232,222 };
            Figures.push_back(curFactory->second->Create(p1, p2, p3, p4, color));
        }
    }

    inputFile_2.close();
    for (int i = 0; i < normal.size(); i++) {
        normal[i] = normal[i] / sqrt(scalar_product(normal, normal));
    }
    for (int i = 0; i < up.size(); i++) {
        up[i] = up[i] / sqrt(scalar_product(up, up));
    }
...
#pragma omp parallel for private(x,y,z)
    for (int i = 0; i < height; ++i) {
        for (int j = 0; j < width; ++j) {
            ...
            direction[1] = y - cam[1];
            direction[2] = z - cam[2];
            std::vector<unsigned char> col = ray_trace(cam, direction, Figures, light);
    ...
    std::cout << "Time is " << elapsed_ms_1 << " ms\n";
    for (auto it : Figures)
        delete it;
    delete Our_map["sphere"];
    delete Our_map["box"];
    delete Our_map["tetraedr"];
    save_image(image);
    free_pixels(image);
    return 0;
}
